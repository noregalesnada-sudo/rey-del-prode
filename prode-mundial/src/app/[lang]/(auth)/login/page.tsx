    ...
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
          ...
          <a href={lp('/auth/forgot-password')} style={{ color: 'var(--accent)', fontSize: '12px', fontWeight: 600, textDecoration: 'none' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: '4px', color: 'var(--text-muted)', fontSize: '13px' }}>
        {t.auth.login.noAccount}{' '}
        <Link href={lp('/register')} style={{ color: 'var(--accent)', fontWeight: 700 }}>{t.auth.login.register}</Link>
      </p>
    </form>
  )
}
