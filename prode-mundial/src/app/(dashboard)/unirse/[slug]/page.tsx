...
export default async function UnirseProdePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: prode } = await supabase
    .from('prodes')
    .select('name, description, owner_id, profiles(username)')
    .eq('slug', slug)
    .single()
  ...
  return (
    <div style={{ maxWidth: '420px', paddingTop: '20px' }}>
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '32px 28px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '13px', marginBottom: '24px' }}>
          <Users size={14} />
