...
const unitStyle: React.CSSProperties = {
  ...
  borderRadius: 8, padding: '10px 14px', minWidth: 60,
}
        ...
        <Fragment key={i}>
          <div style={unitStyle}>
            <span style={{
              ...
              fontSize: 32, lineHeight: 1, color: i === 0 ? '#f5c518' : '#fff',
              fontVariantNumeric: 'tabular-nums',
            }}>
              {String(v).padStart(2, '0')}
            </span>
            <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: 'rgba(255,255,255,0.45)', marginTop: 3 }}>
              {label}
            </span>
          </div>
          {i < 3 && (
            <span style={{ fontFamily: 'var(--font-bebas, var(--font-barlow))', fontSize: 24, color: 'rgba(245,197,24,0.4)', marginBottom: 10 }}>:</span>
          )}
        </Fragment>
      ))}
    </div>
