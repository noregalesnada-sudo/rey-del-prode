...
export const SceneHowItWorksEN: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: "#071428", overflow: "hidden" }}>
      ...
      {/* Full layout: escudo top — card middle — encompass bottom */}
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "56px 60px 52px" }}>

        {/* Escudo */}
        <div style={{ opacity: fadeIn }}>
          <Img
            src={staticFile("escudo_english.png")}
            style={{ height: 260, width: "auto", display: "block", filter: "drop-shadow(0 2px 20px rgba(116,172,223,0.35))" }}
          />
        </div>

        {/* Card */}
        <div style={{
          opacity: fadeIn, transform: `scale(${cardSc})`,
          width: "100%", background: "rgba(10,28,58,0.88)",
          border: "1px solid rgba(116,172,223,0.22)", borderRadius: 20,
        }}>
          {/* Card header */}
          <div style={{
            background: "rgba(7,18,38,0.96)", padding: "24px 36px",
            display: "flex", alignItems: "center", gap: 18,
            borderBottom: "1px solid rgba(116,172,223,0.15)",
            borderRadius: "20px 20px 0 0",
          }}>
            <div style={{
              width: 46, height: 46, borderRadius: "50%", background: "#74ACDF",
              color: "#071428", fontFamily: barlowCondensed, fontWeight: 900, fontSize: 26,
              display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
            }}>1</div>
            <div style={{ fontFamily: barlowCondensed, fontWeight: 700, fontSize: 40, textTransform: "uppercase", letterSpacing: 2, color: "#74ACDF" }}>
              How It Works
            </div>
          </div>
            ...
            {bullets.map((text, i) => (
              ...
              >
                <div style={{
                  ...
                  color: "#74ACDF", fontSize: 20, fontFamily: barlowCondensed, fontWeight: 700,
                  display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2,
                  background: "rgba(116,172,223,0.08)",
                }}>→</div>
                <div style={{ fontFamily: roboto, fontSize: 42, color: "rgba(255,255,255,0.82)", lineHeight: 1.4 }}>
                  {text}
                </div>
              </div>
            ))}
        ...
        {/* Encompass logo */}
        <div style={{ opacity: fadeIn, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <div style={{ width: 120, height: 1, background: "rgba(116,172,223,0.2)" }} />
          <Img
            src={staticFile("logo_encompass.png")}
            style={{ height: 130, width: "auto", display: "block", opacity: 0.88, filter: "brightness(1.1)" }}
          />
        </div>
